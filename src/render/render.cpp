...
static bool HasSameThreadOverlayWork(const SameThreadOverlayState& request, const Config& cfg) {
    if (request.modeHasMirrors || request.modeHasImages || request.modeHasWindowOverlays || request.modeHasBrowserOverlays ||
        ...
        request.showWelcomeToast || request.showPieSpikeAlert) {
        return true;
    }
...
static void RenderSameThreadImGui(const SameThreadOverlayState& request) {
    const bool shouldRenderAnyImGui = request.shouldRenderGui || request.showPerformanceOverlay || request.showProfiler ||
                                      request.showTextureGrid || request.showEyeZoom || request.showPieSpikeAlert;
...
void RenderModeInternal(const ModeConfig* modeToRender, const GLState& s, int current_gameW, int current_gameH, bool skipAnimation,
    ...
    const bool wantAnyImGui = g_shouldRenderGui.load(std::memory_order_relaxed) || g_showPerformanceOverlay.load(std::memory_order_relaxed) ||
                              ...
                              g_showTextureGrid.load(std::memory_order_relaxed) || g_pieSpikeAlertActive.load(std::memory_order_relaxed);
    ...
    const auto populateOverlayState = [&](auto& target) {
        ...
        target.showPieSpikeAlert = g_pieSpikeAlertActive.load(std::memory_order_acquire);
        target.pieSpikeAlertTimeMs = g_pieSpikeLastAlertTimeMs.load(std::memory_order_relaxed);
        if (target.showPieSpikeAlert) {
            memcpy(target.pieSpikeMatchedName, g_pieSpikeMatchedName, sizeof(target.pieSpikeMatchedName));
        }
